...
# Librerías de Ayuda
from src.calculator.utils.logger_setup import notInfiniteLog, debugLogger
...
def updateSessionState(key: str, value: Any) -> bool:
    try:
        ...
        notInfiniteLog('error_updating_session_state', f'Error al actualizar session state: {e}', method='error')
        raise e
...
def getSessionState(key: str) -> Any:
    try:
        ...
        notInfiniteLog('error_getting_session_state', f'Error al obtener session state: {e}', method='error')
        raise e
...
def areSessionStatesDefined(keys: list) -> bool:
    try:
        for key in keys:
            if key not in st.session_state:
                notInfiniteLog(f'missing_{key}', f'Session state no definido: {key}', method='warning')
                return False
        keys_str = ', '.join(keys)
        notInfiniteLog(f'defined_{keys_str}', 'Todos los session states necesarios están definidos', method='debug')
        return True
    except Exception as e:
        notInfiniteLog('error_verifying_session_states', f'Error al verificar session states: {e}', method='error')
        raise e
    ...
    try:
        for key in keys:
            if key not in st.session_state or (key in defaultValues and st.session_state[key] == defaultValues[key]):
                notInfiniteLog(f'invalid_{key}', f'Session state no válido: {key} = {st.session_state.get(key, None)}', method='warning')
                return False
        keys_str = ', '.join(keys)
        notInfiniteLog(f'valid_{keys_str}', 'Todos los session states necesarios son válidos', method='debug')
        return True
    except Exception as e:
        notInfiniteLog('error_verifying_session_states', f'Error al verificar session states: {e}', method='error')
        raise e